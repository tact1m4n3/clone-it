...
const AppData = @import("root").AppData;
...
pub fn init(gpa: std.mem.Allocator) !World {
    ...
    const blocks_renderer = try BlocksRenderer.init(map, gpa);
...
fn mouseToYaw(mouse_position: zlm_f64.Vec2, screen_width: usize, screen_height: usize) gl.Float {
    _ = screen_height;
    ...
    return @floatCast((mouse_position.x / @as(f64, @floatFromInt(screen_width - 1)) * 2.0 - 1.0) * std.math.tau);
}

pub fn update(self: *World, app_data: AppData) void {
    if (app_data.mouse.right_down) |right| {
        if (right.is_event) {
            self.rotation.yaw_on_down = .{
                .relative = mouseToYaw(right.down, app_data.screen.width, app_data.screen.height),
                .absolute = self.rotation.yaw,
            };
        }

        if (self.rotation.yaw_on_down) |yaw_on_down| {
            const yaw_relative = mouseToYaw(app_data.mouse.position, app_data.screen.width, app_data.screen.height);
    ...
    if (app_data.mouse.scroll) |scroll| {
        self.scale += @floatCast(scroll.y_offset / 5.0);
...
const GridRenderer = struct {
    ...
    pub fn uploadData(self: GridRenderer) void {
        ...
        self.vbo.data(Vertex, &vertices, .static_draw);

        gl.vertexAttribPointer(0, 3, .float, false, @sizeOf(Vertex), @offsetOf(Vertex, "position"));
    }

    pub fn update(self: *const GridRenderer, app_data: AppData) void {
        ...
        self.program.uniform1f(screen_aspect_ratio_location, app_data.screen.aspect_ratio);
...
const BlocksRenderer = struct {
    ...
    pub fn init(map: []const MapBlock, gpa: std.mem.Allocator) !BlocksRenderer {
        ...
        const render_map = try mapToRender(map, gpa);
    ...
    pub fn uploadBlockAtlas(self: BlocksRenderer) void {
        ...
        const img = stbi.stbi_load_from_memory(src, src.len, &info.width, &info.height, &info.num_channels, 0) orelse @panic("TODO: Handle error!");
        defer stbi.stbi_image_free(img);
    ...
    pub fn uploadData(self: BlocksRenderer) void {
        inline for (std.meta.fields(BlockKind)) |kind| {
            ...
            self.vaos[kind.value].enableVertexAttribute(0);
            self.vaos[kind.value].enableVertexAttribute(1);
            self.vaos[kind.value].enableVertexAttribute(2);
            self.vaos[kind.value].enableVertexAttribute(3);
            self.vaos[kind.value].enableVertexAttribute(4);

            self.instance_vbos[kind.value].bind(.array_buffer);
            self.instance_vbos[kind.value].data(InstanceVertex, BlockKind.models[kind.value].vertices, .static_draw);
            ...
            self.instance_ebos[kind.value].data(gl.UShort, BlockKind.models[kind.value].indices, .static_draw);

            self.vbos[kind.value].bind(.array_buffer);
            self.vbos[kind.value].data(RenderBlock, self.render_map.blocks[kind.value].items, .static_draw);
    ...
    pub fn update(self: *const BlocksRenderer, app_data: AppData) void {
...
const RenderMap = struct {
    ...
    pub fn init(gpa: std.mem.Allocator) RenderMap {
        var world: RenderMap = undefined;

        for (&world.blocks) |*block| {
            block.* = std.ArrayList(RenderBlock).init(gpa);
        }
...
fn mapToRender(map: []const MapBlock, gpa: std.mem.Allocator) std.mem.Allocator.Error!RenderMap {
    var render_world: RenderMap = RenderMap.init(gpa);
